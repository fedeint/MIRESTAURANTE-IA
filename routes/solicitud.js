...
const singleUpload = multer({
  ...
  limits: { fileSize: 50 * 1024 * 1024 }, // 30 MB max (images compressed in browser, videos limited)
  ...
  singleUpload(req, res, async (err) => {
    if (err instanceof multer.MulterError) {
      const msg = err.code === 'LIMIT_FILE_SIZE'
        ? 'El archivo es demasiado grande. Maximo 50 MB.'
    ...
    try {
      ...
      if (!publicUrl) {
        const MAX_DB_FILE_SIZE = 50 * 1024 * 1024; // 30 MB limit for DB storage
        if (file.buffer.length > MAX_DB_FILE_SIZE) {
          return res.status(400).json({
            error: 'El archivo es demasiado grande para almacenar (' + Math.round(file.buffer.length / 1024 / 1024) + ' MB). Maximo 50 MB. Comprime el archivo e intenta de nuevo.'
          });
        }
        try {
